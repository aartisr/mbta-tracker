 ...
 * Core types for the map-free search interface
 */
...
export interface SearchQuery {
  ...
  filters?: {
    mode?: string; // 'bus', 'subway', 'rail', 'ferry'
    distance_km?: number;
  };
}
...
export interface RouteResult {
  ...
  direction_names: string[]; // e.g., ["Inbound", "Outbound"]
  confidence: number;
  description?: string;
}
...
export interface ArrivalForecast {
  ...
  eta_seconds: number; // Seconds from now
  scheduled_time: number; // Unix timestamp
  delay_seconds: number; // scheduled - predicted
...
export interface VehicleNextStop {
  stop_id: string;
  stop_name: string;
  arrival_time: number;
  eta_seconds: number;
